...
namespace Fraple7
{
	...
	namespace Studio
	{
		...
		{
		public:
			WinWindow(uint32_t width, uint32_t height, const std::string& name);
			~WinWindow() override;
		...
		public:
			void SetPipeLineRef(std::shared_ptr<Core::PipeLineDx> ref) { m_PipeLine = std::move(ref); }
		private:
		...
		private:
			...
			bool m_vSync = false;
		private:
			std::shared_ptr<Core::PipeLineDx> m_PipeLine;
		};
	}
}