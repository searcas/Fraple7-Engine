	...
	namespace Core
	{
		...
		public:
			...
			public:
				QueueDx() = default;
				~QueueDx();
				QueueDx(D3D12_COMMAND_LIST_TYPE type);
				const ComPtr<ID3D12CommandQueue>& GetCmdQueue() const { return m_CommandQueue; }
			public:
				ComPtr<ID3D12GraphicsCommandList2> GetCommandList();
				std::shared_ptr<Command::List>GetCommandListClass();
			...
			private:
				...
				HANDLE m_FenceEvent;
				ThreadSafeQueue<std::shared_ptr<Command::List>> m_AvailableCommandLists;
				D3D12_COMMAND_LIST_TYPE m_CommandListType;
			};
			class List
			{
			public:
				List(D3D12_COMMAND_LIST_TYPE type);
				...
				void CopyStructuredBuffer(StructuredBuffer& structuredBuffer, size_t numElements, const void* bufferData);
				template<typename T>
				...
				void SetGraphicsDnyamicStrucuredBuffer(uint32_t slot, size_t numElements, size_t elementSize, const void* bufferData);
				template <typename T>
				void SetGraphicsDnyamicStrucuredBuffer(uint32_t slot, const std::vector<T>& bufferData)
				{
				...
				bool Close(Command::List& pendingCommandList);
				void Close();
				void Reset();
				void ReleaseTrackedObjects();
				void SetDescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE heapType, ID3D12DescriptorHeap* heap);
				std::shared_ptr<Command::List>GetGeneratedMipsCommandList()const
				{
			...
			private:
				...
				std::shared_ptr<Command::List>m_ComputeCommandList;
